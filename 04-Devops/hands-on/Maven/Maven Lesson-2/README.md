...
- Since we've install Java-11 on the EC2 machine, uncomment the ```properties``` tag of the new pom.xml file.
    $ nano src/main/java/com/clarus/maven/App.java
...
```bash
ec2-user@ip...maven-experiment]$ mvn clean test
```

- Show that there is a new folder named ```target``` in the project root. It's deleteda new one arranged.
...
```
    $ scp -i aws_key.pem -r ~/.aws ec2-user@54.236.25.45:/home/ec2-user/
...
- Run the command below in target folder to copy the contents of the site folder under ```/var/www/html``` folder.
