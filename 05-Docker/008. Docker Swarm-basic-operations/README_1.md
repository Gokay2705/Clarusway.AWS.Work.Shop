...
```bash
sudo hostnamectl set-hostname <node-name-manager-or-worker> #manager-1, bu komut ec2 isim verir.
```

- Check if the docker is active or not from the list of docker info (should be inactive at first).

```bash
docker info #swarn inactiv
```
...
- Learn which node is running `service viz` and check if the visualizer is running by entering `http://<ec2-host-name of this node>:8080` in a browser. (Nothing should be running) # Public IPv4 address adresinin sonuna :8080 ekleyelim hhtp olacak basi.
